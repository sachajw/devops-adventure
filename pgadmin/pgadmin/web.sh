...
# We  will run as allianz user only ;)
# if [ "$EUID" -ne 0 ]
...
if [ ${IS_REDHAT} == 1 ]; then
    ...
    chown allianz: /var/log/pgadmin /var/lib/pgadmin -R
fi
...
if [ $? -eq 0 ]; then
    ...
    case ${RESPONSE} in
        y|Y )
            ...
            if [ $? != 0 ]; then
                echo "Error enabling ${APACHE}. Please check the systemd logs, its just a container. Who cares. ;-)"
            else
            ...
            if [ $? != 0 ]; then
                ...
                echo "You can now start using pgAdmin 4 in web mode at http://127.0.0.1:8080/pgadmin4"
            fi;;
        * ) 
            exit 1;;
    esac
