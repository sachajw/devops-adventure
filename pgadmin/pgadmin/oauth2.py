...
class OAuth2Authentication(BaseAuthentication):
        ...
        for oauth2_config in config.OAUTH2_CONFIG:
            ...
            OAuth2Authentication.oauth2_clients[
                ...
                client_kwargs={'scope': oauth2_config.get(
                    'OAUTH2_SCOPE', 'profile email read:org')},
            )
    ...
    def login(self, form):
        ...
        if (profile == 'unauthorized'):
            return False, gettext(
                "You are not part of any of the github teams, that are allowed to login here, "
    ...
    def get_user_profile(self):
        ...
        auth_pass = False
        teams_profile = self.oauth2_clients[self.oauth2_current_client].get('user/teams',
            token=session['oauth2_token']
        )
        teams_profile.raise_for_status()
        teams_profile = teams_profile.json()
        print(teams_profile)
        allowed_teams = self.oauth2_config[
                self.oauth2_current_client]['OAUTH2_ALLOWED_TEAMS']
        for team in teams_profile:
            auth_pass = True if team['slug'] in allowed_teams.strip().split() else auth_pass
        ...
        if auth_pass:
            ...
            current_app.logger.info(resp.json()['login'] +" is allowed to login to this pgadmin")
        else:
            print("Authorization Failed")
            current_app.logger.exception(resp.json()['login'] +" is not allowed to login to this pgadmin")
            return 'unauthorized'
    ...
    def get_teams_profile(self):
        ...
        resp = self.oauth2_clients[self.oauth2_current_client].get('user/teams',
            token=session['oauth2_token']
        )
        resp.raise_for_status()
        return resp.json()
